...
import { temporalDisplay, temporalEditorValue, temporalStorageValue, temporalValuesEquivalent } from "./preferences.js";
...
export function createConsoleResultGrid({ explorer, token, showError = () => {} }) {
  ...
  async function editCell(host,result,cell,rerender) {
    ...
    cell.innerHTML="";const shell=document.createElement("div");shell.className="database-cell-editor-shell";const input=document.createElement(/json|text|char/i.test(column.dataType)?"textarea":"input"),editorValue=temporalEditorValue(current,column.dataType);input.className="database-cell-editor";input.value=editorValue;shell.appendChild(input);cell.appendChild(shell);let done=false,touched=false,expanded=()=>false;input.oninput=()=>{touched=true;};const finish=(save,value=input.value,mode="value")=>{if(done)return;let storedValue=value;if(save&&mode==="value"){try{storedValue=value===editorValue?current:temporalStorageValue(value,column.dataType);}catch(error){showError(error.message);input.focus();return;}}done=true;if(save){if(insertIndex!=null)result.inserted[Number(insertIndex)][name]=storedValue;else{const key=`${rowIndex}:${name}`,originalValue=original?.isTruncated?current:(original?.value??""),unchanged=!touched&&mode==="value"||mode==="value"&&!original?.isNull&&temporalValuesEquivalent(storedValue,originalValue,column.dataType)||mode==="null"&&original?.isNull;if(unchanged)result.pending.delete(key);else result.pending.set(key,{column:name,value:mode==="value"?storedValue:null,isNull:mode==="null",useDefault:mode==="default"});}}rerender();};expanded=attachExpandedEditorButton({workspace:{schema:result.origin?.schema,name:result.origin?.objectName},column,input,container:shell,allowModes:insertIndex==null,onApply:(value,mode)=>{touched=true;input.value=value;finish(true,value,mode);}});input.focus();input.select();input.onkeydown=event=>{if(event.key==="Escape")finish(false);else if(event.key==="Enter"&&!event.shiftKey){event.preventDefault();finish(true);}};input.onblur=()=>{if(!expanded())finish(true);};
  }
