...
(() => {
  ...
  async function loadRows(ws, message = "Đang tải dữ liệu…") {
    clearError(); ws.queryAbort?.abort(); const controller = new AbortController(); ws.queryAbort = controller; setGridLoading(ws, true, message);
    try {
  ...
  function renderDataWorkspace(ws) {
    ...
    const tableWidth = 58 + visible.reduce((total, { c }) => total + (ws.widths[c.name] || 180), 0);
    stage.innerHTML = `<div class="database-data-workspace">${dataToolbar(ws)}
      ...
      <div class="database-workspace-grid-shell"><div class="database-workspace-grid-scroll" aria-busy="${ws.rowsLoading ? "true" : "false"}"><table class="database-workspace-grid" style="width:${tableWidth}px"><colgroup><col style="width:58px">${visible.map(({c})=>`<col data-column-width="${html(c.name)}" style="width:${ws.widths[c.name]||180}px">`).join("")}</colgroup><thead><tr><th class="database-row-number">#</th>${visible.map(({c}) => columnHeaderHtml(ws,c)).join("")}</tr></thead>
      <tbody>${ws.rows.map((row, ri) => {const heightKey=`page:${ws.page}:${ri}`,height=ws.rowHeights[heightKey]||defaultRowHeight;return `<tr data-row="${ri}" data-visual-row="${ri}" class="${ws.deleted.has(ri) ? "is-deleted " : ""}${ws.activeRow===ri?"is-active-row ":""}${height!==defaultRowHeight?"is-row-resized":""}" style="height:${height}px"><th class="database-row-number" data-select-row="${ri}"><span class="database-row-number-content"><span>${(ws.page - 1) * ws.pageSize + ri + 1}</span><button type="button" data-inspect-row="${ri}" aria-label="Xem chi tiết row ${(ws.page - 1) * ws.pageSize + ri + 1}" title="Row details & placement"><i class="fa fa-info-circle" aria-hidden="true"></i></button></span><i class="database-row-resizer" data-row-height-key="${heightKey}" title="Kéo để đổi chiều cao · double-click để reset"></i></th>${visible.map(({c,i}) => cellHtml(ws,row,ri,c,i)).join("")}</tr>`;}).join("")}${ws.inserted.map((row, ii) => {const visualRow=ws.rows.length+ii,heightKey=`insert:${ii}`,height=ws.rowHeights[heightKey]||defaultRowHeight;return `<tr class="is-inserted ${ws.activeRow===visualRow?"is-active-row ":""}${height!==defaultRowHeight?"is-row-resized":""}" data-insert="${ii}" data-visual-row="${visualRow}" style="height:${height}px"><th class="database-row-number" data-select-row="${visualRow}"><span class="database-row-number-content"><span>+</span><button type="button" data-inspect-insert="${ii}" aria-label="Xem chi tiết row chưa lưu" title="Row details"><i class="fa fa-info-circle" aria-hidden="true"></i></button></span><i class="database-row-resizer" data-row-height-key="${heightKey}" title="Kéo để đổi chiều cao · double-click để reset"></i></th>${visible.map(({c,i}) => `<td tabindex="0" data-cell data-row="${visualRow}" data-col="${i}" data-insert-cell="${ii}" data-column="${html(c.name)}" class="${ws.selected.has(`${visualRow}:${i}`)?"is-selected":""}">${html(row[c.name] ?? "")}</td>`).join("")}</tr>`;}).join("")}</tbody></table></div><div class="database-grid-loading ${ws.rowsLoading ? "" : "hidden"}" data-grid-loading role="status" aria-live="polite"><div><div class="database-spinner"></div><p>${html(ws.loadingMessage || "Đang tải dữ liệu…")}</p></div></div></div>
