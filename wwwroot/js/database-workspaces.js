(() => {
  ...
  async function loadRows(ws) {
    ...
    ws.observedMinimum=Math.max(ws.observedMinimum||0,knownMinimum);ws.selected.clear();ws.loaded=true;if(activeKey===ws.key)renderDataWorkspace(ws);
  }
  ...
  function dataToolbar(ws) {
    ...
    return `<div class="database-grid-toolbar">
      ...
      <details class="database-toolbar-menu database-range-menu"><summary title="Chọn range hoặc số dòng mỗi trang">${start}–${end}⌄</summary><div><p>Page ranges</p>${pageRangeOptions(ws)}<hr><p>Rows per page</p>${[25,50,100].map(size=>`<button data-page-size-option="${size}" class="${ws.pageSize===size?"is-active":""}">${size} rows</button>`).join("")}</div></details>
      <button class="database-total-count" data-total-count ${nodeId?"disabled":""} title="${nodeId?"Exact count chỉ chạy trên coordinator":"Bấm để lấy exact count"}">${ws.counting?"Cancel count":`of ${totalLabel}${approximate}`}</button>
      ...
      <label>Auto <select data-auto-refresh><option value="0">Off</option><option value="5" ${ws.autoRefresh===5?"selected":""}>5s</option><option value="15" ${ws.autoRefresh===15?"selected":""}>15s</option><option value="30" ${ws.autoRefresh===30?"selected":""}>30s</option><option value="60" ${ws.autoRefresh===60?"selected":""}>60s</option></select></label>
  ...
  function renderDataWorkspace(ws) {
    ...
    stage.innerHTML = `<div class="database-data-workspace">${dataToolbar(ws)}
      ...
      <div class="database-workspace-grid-scroll"><table class="database-workspace-grid"><thead><tr><th class="database-row-number">#</th>${visible.map(({c}) => columnHeaderHtml(ws,c)).join("")}</tr></thead>
  ...
  function bindDataWorkspace(ws) {
    ...
    stage.querySelectorAll("[data-page-size-option]").forEach(button=>button.onclick=()=>{ws.pageSize=Number(button.dataset.pageSizeOption);ws.page=1;ws.observedMinimum=0;loadRows(ws).catch(reportError);});
  ...
  function applyFilter(ws) { const nextWhere=stage.querySelector("[data-where]").value.trim();ws.orderBy=stage.querySelector("[data-order]").value.trim();if(nextWhere!==ws.where){ws.exactCount=null;ws.observedMinimum=0;}ws.where=nextWhere;ws.page=1;loadRows(ws).catch(reportError);persist(); }
  ...
  async function saveRows(ws){ try{const grouped=new Map();ws.pending.forEach((value,key)=>{const ri=Number(key.split(":")[0]);if(!grouped.has(ri))grouped.set(ri,[]);grouped.get(ri).push(value);});const body={schema:ws.schema,objectName:ws.name,
    ...
    await jsonApi(explorer.dataset.workspaceApplyUrl,body);ws.pending.clear();ws.deleted.clear();ws.inserted=[];ws.exactCount=null;ws.observedMinimum=0;setDirty(ws,false);await loadRows(ws);
