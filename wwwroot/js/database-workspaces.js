...
import { initializeDatabasePreferences, temporalDisplay, temporalEditorValue, temporalStorageValue, temporalValuesEquivalent } from "./database-workspaces/preferences.js";
...
(() => {
  ...
  function cellHtml(ws, row, ri, column, ci) {
    ...
    const display=pending?.useDefault?'<span class="database-default">DEFAULT</span>':pending?.isNull||(!pending&&cell.isNull)?'<span class="database-null">NULL</span>':temporalValueHtml(value,column.dataType);
  ...
  function temporalValueHtml(value,dataType){return `<span data-temporal-value="${html(value??"")}" data-temporal-type="${html(dataType??"")}">${html(temporalDisplay(value??"",dataType))}</span>`;}
  ...
  async function editCell(ws,cell){
    ...
    cell.innerHTML="";const container=document.createElement("div");container.className="database-cell-editor-shell";let input;if(/^bool/i.test(col.dataType)){input=document.createElement("select");["true","false"].forEach(value=>{const option=document.createElement("option");option.value=value;option.textContent=value;input.appendChild(option);});}else{input=document.createElement(/json|text|char/i.test(col.dataType)?"textarea":"input");if(col.isNumeric)input.type="number";}const editorValue=temporalEditorValue(current,col.dataType);input.className="database-cell-editor";input.value=editorValue;container.appendChild(input);cell.appendChild(container);
    let done=false,touched=false,isExpanded=()=>false;input.oninput=()=>{touched=true;};input.onchange=()=>{touched=true;};const finish=(save,mode="value",nextValue=input.value)=>{if(done)return;let storedValue=nextValue;if(save&&mode==="value"){try{storedValue=nextValue===editorValue?current:temporalStorageValue(nextValue,col.dataType);}catch(error){showError(error.message);input.focus();return;}}done=true;if(save){const untouched=mode==="value"&&!pending&&!touched,unchanged=untouched||mode==="value"&&!originalCell.isNull&&temporalValuesEquivalent(storedValue,originalValue,col.dataType)||mode==="null"&&originalCell.isNull;if(unchanged)ws.pending.delete(key);else ws.pending.set(key,{column:col.name,value:mode==="value"?storedValue:null,isNull:mode==="null",useDefault:mode==="default"});syncDirty(ws);}renderDataWorkspace(ws);};
  ...
  function editInsertedCell(ws,cell){const col=ws.columns.find(c=>c.name===cell.dataset.column);if(!col||col.isGenerated||col.isIdentity)return;const row=ws.inserted[Number(cell.dataset.insertCell)],current=row[col.name]??"",editorValue=temporalEditorValue(current,col.dataType);cell.innerHTML="";const container=document.createElement("div");container.className="database-cell-editor-shell";const input=document.createElement("input");input.className="database-cell-editor";input.value=editorValue;container.appendChild(input);cell.appendChild(container);let done=false,isExpanded=()=>false;const finish=(save,value=input.value)=>{if(done)return;let storedValue=value;if(save){try{storedValue=value===editorValue?current:temporalStorageValue(value,col.dataType);}catch(error){showError(error.message);input.focus();return;}}done=true;if(save){row[col.name]=storedValue;setDirty(ws,true);}renderDataWorkspace(ws);};isExpanded=attachExpandedEditorButton({workspace:ws,column:col,input,container,allowModes:false,onApply:value=>{input.value=value;finish(true,value);}});input.focus();input.select();input.onkeydown=e=>{if(e.key==="Enter")finish(true);if(e.key==="Escape")finish(false);};input.onblur=()=>{if(!isExpanded())finish(true);};}
  ...
  document.addEventListener("database:open-tree-object",event=>{const object=event.detail?.node;if(!object)return;document.querySelectorAll("[data-database-object]").forEach(x=>x.classList.remove("is-active"));object.classList.add("is-active");openObject(object.dataset.schema,object.dataset.table,object.dataset.nodeKind==="sequence"?"ddl":"data");});
