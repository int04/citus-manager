(() => {
  ...
  function changePageSize(ws,size){if(!Number.isInteger(size)||size<1||size>500){showError("Rows per page phải là số nguyên từ 1 đến 500.");return;}ws.pageSize=size;ws.page=1;ws.observedMinimum=0;loadRows(ws).catch(reportError);persist();}
