...
(() => {
  ...
  async function loadRows(ws, message = "Đang tải dữ liệu…") {
  ...
  function bindDataWorkspace(ws) {
    ...
    stage.querySelectorAll("[data-page]").forEach(b => b.onclick = () => { ws.page = Number(b.dataset.page); loadRows(ws, "Đang chuyển trang…").catch(reportError); });
    ...
    stage.querySelector("[data-last-page]").onclick = () => { if(ws.exactCount != null){ws.page=Math.max(1,Math.ceil(ws.exactCount/ws.pageSize));loadRows(ws,"Đang chuyển trang…").catch(reportError);} };
    stage.querySelector("[data-refresh]").onclick = () => ws.dirty ? showError("Save/Revert thay đổi trước khi refresh.") : loadRows(ws,"Đang tải lại dữ liệu…").catch(reportError);
  ...
  function sortColumn(ws,name,multi){cycleGridSort(ws,name,multi);ws.page=1;loadRows(ws,"Đang sắp xếp dữ liệu…").catch(reportError);}
  ...
  function updateFooter(ws){if(!ws){statistics.textContent="0 row · 0 column · 0 cell";return;}const consoleResult=ws.type==="sql"&&ws.activeResult!=null?ws.results?.[ws.activeResult]:null,schema=consoleResult?.origin?.schema||ws.schema||"",objectName=consoleResult?.origin?.objectName||ws.name;footerPath.innerHTML=`<span>Database</span><b>›</b><span>${html(nodeId?"worker":"coordinator")}</span><b>›</b><span>${html(schema)}</span><b>›</b><strong>${html(objectName)}</strong><b>›</b><span>${html(consoleResult?`result ${ws.activeResult+1}`:ws.type)}</span>`;if(consoleResult){const valueAt=(row,column)=>{const name=consoleResult.columns[column]?.name,pending=consoleResult.pending?.get(`${row}:${name}`);if(pending)return pending.isNull?null:pending.useDefault?"DEFAULT":pending.value;if(row>=consoleResult.rows.length)return consoleResult.inserted?.[row-consoleResult.rows.length]?.[name]??null;return consoleResult.rows[row]?.[column]?.value??null;},value=gridSelectionStatistics(consoleResult.selected||new Set(),valueAt);statistics.textContent=`${value.rows} row · ${value.columns} column · Count ${value.cells}${value.numeric?` · Sum ${value.sum.toLocaleString()} · Avg ${value.average.toLocaleString()} · Min ${value.minimum.toLocaleString()} · Max ${value.maximum.toLocaleString()}`:""}`;return;}if(ws.type!=="data"){statistics.textContent=ws.type.toUpperCase();return;}const value=gridSelectionStatistics(ws.selected,(row,column)=>workspaceCellValue(ws,row,column));statistics.textContent=`${value.rows} row · ${value.columns} column · Count ${value.cells}${value.numeric?` · Sum ${value.sum.toLocaleString()} · Avg ${value.average.toLocaleString()} · Min ${value.minimum.toLocaleString()} · Max ${value.maximum.toLocaleString()}`:""}`;}
